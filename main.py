...
@app.post("/upload/")
async def upload_files(files: List[UploadFile] = File(...), sec_files: List[UploadFile] = File(...)):
    try:
        if not files and not sec_files:
            raise HTTPException(status_code=400, detail="At least one file must be uploaded.")

        text = ""
        for sec_file in sec_files:
            contents = await sec_file.read()
            filename, file_extension = os.path.splitext(sec_file.filename)
            text += get_file_text(contents, file_extension[1:])
        for uploaded_file in files:
            contents = await uploaded_file.read()
            filename, file_extension = os.path.splitext(uploaded_file.filename)
...
async def analyze_image(input_prompt: str = input_prompt, image: UploadFile = File(...), input_text: str = Form(...)):  # str = Form(...)
    try:
        input_image_data = process_uploaded_image(image)
        response = get_gemini_response(input_text, [input_image_data], input_prompt)
